...
import type { Finding, FindingBundle, StructuredFindingDraft } from '../types/hse';
...
export async function createFindingBundle(fieldEntryId: string, draft: StructuredFindingDraft, dueAt: string | null) {
...
export async function getFindingBundle(id: string): Promise<FindingBundle> {
  const [findingResult, actionsResult, evidenceResult, eventsResult] = await Promise.all([
    ...
    supabase.from('evidence_files').select('id,file_name,mime_type,storage_path,created_at').eq('finding_id', id).order('created_at', { ascending: false }),
...
export async function addEvidence(workspace: Workspace, findingId: string, upload: { path: string; fileName: string; byteSize: number }, mimeType: string) { const uid = await userId(); const { error } = await supabase.from('evidence_files').insert({ organization_id: workspace.organizationId, finding_id: findingId, storage_bucket: 'hse-evidence', storage_path: upload.path, file_name: upload.fileName, mime_type: mimeType, byte_size: upload.byteSize, uploaded_by: uid }); if (error) throw error; }

export async function listPendingReminders(workspace: Workspace) { const { data, error } = await supabase.from('reminders').select('id,scheduled_for,status,finding_id,findings(title)').eq('organization_id', workspace.organizationId).eq('status', 'pending').order('scheduled_for').limit(100); if (error) throw error; return data || []; }
